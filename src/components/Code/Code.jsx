...
const Code = ({ rowsNum, rowsArr }) => {
  ...
  const handleAnimationStart = row => {
    if (row === 0) {
      dispatch({
        type: "STOP_CURSOR",
      })
              ...
              return (
                <GridItem key={row}>
                  <AbsoluteDiv
                    ...
                    onAnimationStart={() => handleAnimationStart(row)}
                    doneWriting={
                      state.assignRef > row ||
                  ...
                  >
                    {state.assignRef === row ? (
                      <AnimatedCursor show={state.show} />
                    ) : null}
                  </AbsoluteDiv>
                  {row === 0 ? (
